...
author: "Candidate numbers: 74, 79, 85"
format:
  pdf:
...
recommended_stores <- Vinmonopolet_market %>%
  ...
  mutate(
    ...
    across(where(is.numeric) & !c(dist_nearest, sales), ~round(., 3))  # Round other numeric columns to 3 decimals
  )
...
```{r table_3, message = FALSE, warning = FALSE}
