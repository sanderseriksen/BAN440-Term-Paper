...
```{r file_path, message = FALSE, warning = TRUE}
...
```{r demand_estimation, message = FALSE, warning = TRUE}
...
```{r demand_summary, message = FALSE, warning = TRUE}

stargazer(reg1, type = "text")

```

```{r logit, message = FALSE, warning = TRUE}
...
recommended_stores_rounded[numeric_columns] <- lapply(recommended_stores_rounded[numeric_columns], round, 3)
